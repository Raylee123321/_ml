...
class SecurityManager:
    """Handles static blacklists, sensitive paths, environment scrubbing, and violation logging."""
    ...
    BLOCKED_PATTERNS = [
        ...
        r"\brmdir\s+/[sS]",             # rmdir /s (Windows)
        r"\brd\s+/[sS]",                # rd /s (Windows)
        r"\bshred\b",
        r"\bmkfs\b",
        r"\bdd\b",
        # Power management
        ...
        r"\buseradd\b",
        r"\buserdel\b",
        # Process killing (anti-analysis/DoS)
        r"\btaskkill\b",
        r"\bkillall\b",
        # Piping command to shell directly
    ...
    SENSITIVE_PATHS = [
        ...
        r"[Cc]:\\Windows\\System32\\drivers\\etc\\hosts", # static DNS config
        r"SAM", # Security Accounts Manager
        # Linux
        r"/etc/passwd",
        r"/etc/shadow",
        r"/etc/sudoers",
    ...
    def __init__(self, log_path: str = "security_violations.log"):
        self.log_path = log_path

    def log_violation(self, cmd: str, reason: str):
        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        log_message = f"[{timestamp}] VIOLATION BLOCKED: Command='{cmd}' | Reason='{reason}'\n"
        print(f"⚠️  [SecurityManager] {reason}")
        try:
    ...
    def is_safe_command(self, cmd: str) -> tuple[bool, str]:
        ...
        for pattern in self.BLOCKED_PATTERNS:
            if re.search(pattern, cmd, re.IGNORECASE):
                reason = f"匹配黑名單規則: {pattern}"
                self.log_violation(cmd, reason)
                return False, reason
        
        # 2. Check sensitive path access
        for pattern in self.SENSITIVE_PATHS:
            if re.search(pattern, cmd, re.IGNORECASE):
                reason = f"嘗試存取敏感檔案/目錄: {pattern}"
                self.log_violation(cmd, reason)
                return False, reason
                
        return True, ""

    def scrub_env(self) -> dict:
        """Returns a copy of environment variables excluding secrets."""
        clean_env = os.environ.copy()
        for key in list(clean_env.keys()):
            if any(kw in key.upper() for kw in self.SENSITIVE_ENV_KEYS):
...
<assistant>{assistant_response}</assistant>"""
    ...
    async def execute_shell(self, cmd: str, cwd: str = None) -> tuple[int, str]:
        ...
        # 1. 靜態規則與敏感路徑攔截
        is_hard_safe, hard_reason = self.security_manager.is_safe_command(cmd)
        if not is_hard_safe:
            return -1, f"安全阻止：{hard_reason}"
        
        # 2. LLM 安全審查
        is_safe, reason = await self.review_command(cmd)
        if not is_safe:
            print(f"⚠️  被安全審查阻止: {reason}")
            self.security_manager.log_violation(cmd, f"LLM 審查阻止: {reason}")
        ...
        if needs_access:
            if path in self.outside_access_granted:
                ...
                if not self.ask_outside_access(path):
                    ...
                    self.security_manager.log_violation(cmd, f"拒絕越界存取：{path}")
                    return -1, f"拒絕：{path}"
                self.outside_access_granted.add(path)
        
        try:
            # 3. 環境變數淨化
            clean_env = self.security_manager.scrub_env()
